...
// Enqueue a set of piles for consensus computation
//
pair<vector<poa_gpu_utils::Task<vector<string>>>, unordered_map<kmer, unsigned>> 
computeConsensuses_enqueue(
		std::string& readId, std::vector<std::string> & piles, std::pair<unsigned, unsigned>& pilesPos,
		unsigned& minSupport, 
		unsigned& merSize, 
	...
	return MSABMAAC_gpu_enqueue(tid, piles, merSize, bmeanSup, solidThresh, minAnchors, maxMSA, path);
	
}

//Dequeue consensus tasks and do postprocessing on the read
//
std::pair<std::string, std::unordered_map<kmer, unsigned>> 
computeConsensuses_dequeue(
		pair<vector<poa_gpu_utils::Task<vector<string>>>, unordered_map<kmer, unsigned>> &tasks, 
	...
	vector<vector<string>> result = MSABMAAC_gpu_dequeue(tasks.first, maxMSA, path);
	
	if(result.size() == 0){
		return std::make_pair("", tasks.second);
	}	

	string corTpl = result[0][0];

	// Polish the consensus
	std::vector<std::pair<std::string, std::string>> corList;
	if (corTpl.length() >= merSize) {
		corTpl = weightConsensus(corTpl, piles, tasks.second, merSize, windowSize, solidThresh);
		corTpl = polishCorrection(corTpl, tasks.second, merSize, solidThresh);
	}

	return std::make_pair(corTpl, tasks.second);
}
...
//Read preprocessing up to the enqueue for MSA
//
...
processRead_enqueue(
	...
	//Enqueue consensuses for all the piles
	...
	for (i = 0; i < pilesPos.size(); i++) {
		...
		pilesTasks[i] = computeConsensuses_enqueue(readId, curPile, pilesPos[i], minSupport, merSize, commonKMers, minAnchors, 
...
std::pair<std::string, std::string> processRead_dequeue(
		vector<pair<vector<poa_gpu_utils::Task<vector<string>>>, unordered_map<kmer, unsigned>>> &pilesTasks, 
	...
	vector<std::string> consensuses(pilesTasks.size()); 
	vector<std::unordered_map<kmer, unsigned>> merCounts(pilesTasks.size());
	std::string readId = alignments.begin()->qName;
	...
	for (int i = 0; i < pilesPos.size(); i++) {  
		curPile = getAlignmentPileSeq(alignments, minSupport, windowSize, windowOverlap, sequences, pilesPos[i].first, pilesPos[i].second, merSize, maxSupport);
		templates[i] = curPile[0];
		auto consensusPair = computeConsensuses_dequeue(pilesTasks[i], curPile, merSize, maxMSA, solidThresh, windowSize, path);
		consensuses[i] = consensusPair.first;
		merCounts[i] = consensusPair.second;
	}
